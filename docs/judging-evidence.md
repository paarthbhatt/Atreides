...
| --- | --- | --- |
| Technological implementation | A real stdio MCP broker discovers configured tools, evaluates policy before forwarding, and records hash-chained receipts. The test suite includes an actual spawned upstream MCP fixture—not only mocked functions. | `apps/gateway/src/upstream.ts`, `apps/gateway/src/proxy.ts`, `apps/gateway/test/upstream.test.ts` |
...
5. Run `npm run test --workspace=@atreides/gateway` to see both blocked and allowed paths, including real stdio upstream MCP discovery/call coverage.
